...
Functions:
1. run_city_batch: Timer-triggered function (every 20 minutes)
   - Collects current weather data for all cities (parallel)
   - Retrieves air quality information (parallel)
   - Downloads and processes NASA satellite images (parallel)

2. get_hourly_forecast: Timer-triggered function (every 12 hours)
   - Fetches extended weather forecasts (parallel)
...
- ThreadPoolExecutor for parallel city processing (10 workers)
- Per-thread database connections for thread safety (pyodbc is not thread-safe)
- Icon pre-fetched once per batch run and shared across threads
- Parallel blob downloads for animation generation
...
def process_city_nasa(
    ...
    Args:
        ...
        icon_bytes (bytes): Pre-fetched location marker icon bytes (downloaded once per batch run)
        ...
        if response.status_code == 200:
            ...
            if img_tag and "src" in img_tag.attrs:
                ...
                if img_response.status_code == 200:
                    ...
                    # Add location marker icon to the processed image using pre-fetched bytes
                    modified_image_data = add_icon_to_image(image_data, icon_bytes)
...
# =============================================================================
# WORKER FUNCTIONS - PARALLEL CITY PROCESSING
# =============================================================================

def _process_city_batch_worker(city: dict, apikey: str, connection_string: str, blob_service_client, container_name: str, icon_bytes) -> None:
    """
    Thread worker: processes weather, air quality, and NASA satellite data for one city.

    Opens its own database connection (pyodbc is not thread-safe) and commits after
    the weather and air quality inserts before proceeding to blob storage operations.

    Args:
        city (dict): City record from Data API (CityCode, CityName, Latitude, Longitude)
        apikey (str): OpenWeatherMap API key
        connection_string (str): SQL Server connection string
        blob_service_client: Azure Blob Storage service client (thread-safe)
        container_name (str): Blob storage container name
        icon_bytes (bytes): Pre-fetched location marker icon bytes
    """
    import pyodbc
    ...
    try:
        ...
        try:
            process_city_nasa(blob_service_client, container_name, icon_bytes, city_code, latitude, longitude)
        except Exception as e:
            logging.error(f"NASA processing failed for {city_code}: {str(e)}")

    except Exception as e:
        logging.error(f"Worker error for {city_code}: {str(e)}")
    finally:
        cursor.close()
        conn.close()


def _process_city_forecast_worker(city: dict, apikey: str, connection_string: str) -> None:
    """
    Thread worker: fetches and stores the 24-hour hourly forecast for one city.

    Opens its own database connection (pyodbc is not thread-safe) and commits after
    all forecast records for the city are inserted.

    Args:
        city (dict): City record from Data API (CityCode, CityName, Latitude, Longitude)
        apikey (str): Azure Maps subscription key
        connection_string (str): SQL Server connection string
    """
    import pyodbc
    import requests
    ...
    try:
        ...
        for forecast in forecast_data.get("forecasts", []):
            ...
            cursor.execute(insert_query, (
                ...
                forecast.get("iconPhrase"),  # API doesn't have "shortPhrase", using iconPhrase
...
def run_city_batch(timer: func.TimerRequest) -> None:
    ...
    2. Initializes Azure Blob Storage with managed identity
    3. Pre-fetches the location marker icon once (shared across all city threads)
    4. Processes all cities in parallel using ThreadPoolExecutor (10 workers):
       - Each worker opens its own DB connection (pyodbc thread safety)
       - Collects current weather data from OpenWeatherMap
       - Retrieves air quality information and AQI levels
       - Commits DB writes, then downloads and processes NASA GOES satellite imagery
       - Generates animated satellite imagery timeline

    Error Handling:
    - Individual city failures don't stop processing of other cities
    - Database connection errors are logged and handled gracefully per worker
    ...
    Performance:
    - Parallel city processing (10 concurrent workers) vs. previous sequential loop
    - Icon downloaded once per batch run instead of once per city
    - Parallel blob downloads within animation generation
    """
    ...
    try:
        ...
        # Pre-fetch the location marker icon once and share across all city workers
        icon_bytes = None
        try:
            icon_response = _requests.get(icon_url, timeout=10)
            if icon_response.status_code == 200:
                icon_bytes = icon_response.content
                logging.info("✅ Location marker icon pre-fetched successfully.")
            else:
                logging.warning(f"⚠️ Failed to pre-fetch icon (HTTP {icon_response.status_code}), proceeding without overlay.")
        except Exception as e:
            logging.warning(f"⚠️ Could not pre-fetch icon: {e}. Proceeding without overlay.")
        ...
        logging.info(f'Successfully fetched {len(cities_data)} cities from API. Processing in parallel.')

        # Process all cities in parallel — each worker manages its own DB connection
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {
                executor.submit(
                    _process_city_batch_worker,
                    city, apikey, connection_string, blob_service_client, container_name, icon_bytes
                ): city.get('CityCode')
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logging.error(f"Unhandled error in batch worker for {city_code}: {str(e)}")

        logging.info("✅ run_city_batch completed for all cities.")
...
def add_icon_to_image(main_image, icon_bytes):
    """
    Add a location marker icon overlay to a satellite image.

    This function uses pre-fetched icon bytes and overlays the marker on the center
    of a satellite image to provide geographic reference for users.

    Args:
        main_image: PIL Image object (satellite image)
        icon_bytes (bytes): Pre-fetched icon image bytes (downloaded once per batch run)

    Returns:
        bytes: JPEG-encoded image data with icon overlay

    Processing Details:
        - Uses pre-fetched icon bytes (no network call)
...
def generate_animation_for_city(city_code: str, blob_service_client, container_name: str) -> bool:
    ...
    Animation Process:
        ...
        - Downloads all frames in parallel (up to 5 concurrent downloads)
    ...
    Memory Optimization:
        - Parallel blob downloads reduce total I/O wait time
    ...
    try:
        ...
        # Reverse to show chronological progression (oldest to newest)
        blobs_to_use = list(reversed(blobs[:10]))

        # Download all frames in parallel to reduce I/O wait time
        def _download_frame(blob):
            blob_client = blob_service_client.get_blob_client(container=container_name, blob=blob.name)
            return blob.name, blob_client.download_blob().readall()

        raw_frames = {}
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {executor.submit(_download_frame, blob): blob for blob in blobs_to_use}
            for future in as_completed(futures):
                try:
                    name, data = future.result()
                    raw_frames[name] = data
                except Exception as e:
                    logging.warning(f"Failed to download blob for animation ({city_code}): {e}")

        # Assemble frames in chronological order (blobs_to_use order)
        files = []
        for blob in blobs_to_use:
            blob_data = raw_frames.get(blob.name)
...
@app.schedule(schedule="0 10,22 * * *", arg_name="hourlyTimer", run_on_startup=False, use_monitor=False)
def get_hourly_forecast(hourlyTimer: func.TimerRequest) -> None:
    ...
    Schedule: Every 12 hours (CRON: "0 10,22 * * *")

    Process Flow:
    1. Retrieves list of cities from Data API
    2. Processes all cities in parallel using ThreadPoolExecutor (10 workers):
       - Each worker opens its own DB connection (pyodbc thread safety)
       - Calls Azure Maps Weather API for hourly forecasts (24 hours ahead)
       - Extracts comprehensive forecast data (temperature, humidity, wind, precipitation)
       - Stores forecast records in WeatherForecast table and commits
    ...
    - 24-hour duration for detailed short-term forecasting
    ...
    - Per-worker commits for parallel safety
    """
    ...
    try:
        ...
        logging.info(f'Successfully fetched {len(cities_data)} cities. Processing forecasts in parallel.')

        # Process all cities in parallel — each worker manages its own DB connection
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {
                executor.submit(_process_city_forecast_worker, city, apikey, connection_string): city.get('CityCode')
                for city in cities_data
            }
            for future in as_completed(futures):
                city_code = futures[future]
                try:
                    ...
                    logging.error(f"Unhandled error in forecast worker for {city_code}: {str(e)}")

        logging.info("✅ get_hourly_forecast completed for all cities.")
